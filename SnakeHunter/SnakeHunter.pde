  ...
  void snakeMovement() {
    for (int i = 0; i < snakes.size (); i++) {
      ...
      if (millis() - moveTime >= 500) {
        ...
        Unit end = snakes.get(i).getUnits().get(snakes.get(i).getUnits().size()-1);

        tiles[end.getYcor()][end.getXcor()].changeSnake(false); //old spot no longer has snake
        
        Tile head = tiles[snake.getUnits().get(0).getYcor()][snake.getUnits().get(0).getXcor()];
        Tile food = getClosestFood(head);
        int dir = findDirection(head.getXcor(), head.getYcor(), food.getXcor(), food.getYcor());
        snake.move(dir);

        Unit start = snakes.get(i).getUnits().get(0); //new spot gets snake
        tiles[start.getYcor()][start.getXcor()].changeSnake(true);
  ...
  int findDirection(int startX, int startY, int endX, int endY) {
    ...
    while (!open.isEmpty ()) {
      ...
      if (goldenTile.equals(tiles[endY][endX])) { //search up list of parents
        ...
        closed.add(0,open.remove(open.indexOf(goldenTile)));
